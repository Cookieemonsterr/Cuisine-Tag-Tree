...
function renderSubpageTags() {
  ...
  const infoBox = document.getElementById("subpageInfo");

  if (appState.selectedCuisines.length === 0) {
    grid.innerHTML = "";
    infoBox.innerHTML =
      '<p class="warning">👉 Select cuisines above to see related subpage tags</p>';
    return;
  }

  infoBox.innerHTML =
    '<p class="success">✓ Available subpage tags for your selection</p>';
  ...
  relatedSubpageTags.forEach((tag) => {
    ...
    if (appState.selectedSubpageTags.find((t) => t.id === tag.id)) {
      btn.classList.add("active");
    }

    btn.addEventListener("click", () => toggleSubpageTag(tag));
    grid.appendChild(btn);
  });
...
function toggleSubpageTag(tag) {
  const index = appState.selectedSubpageTags.findIndex((t) => t.id === tag.id);

  if (index > -1) {
    appState.selectedSubpageTags.splice(index, 1);
    showToast(`${tag.name} removed`, "info");
  } else {
    if (appState.selectedSubpageTags.length < appState.maxSubpageTags) {
      appState.selectedSubpageTags.push(tag);
      showToast(`${tag.name} added`, "success");
    } else {
      showToast(
        `Maximum ${appState.maxSubpageTags} subpage tags can be selected`,
        "warning",
      );
      return;
    }
...
function updateAllCounters() {
  ...
  document.getElementById("subpageCount").textContent =
    appState.selectedSubpageTags.length;
}
...
function updateSummary() {
  ...
  const previewSubpageTags = document.getElementById("previewSubpageTags");
  previewSubpageTags.innerHTML =
    appState.selectedSubpageTags.length > 0
      ? appState.selectedSubpageTags.map((t) => `<li>${t.name}</li>`).join("")
...
function updateJsonPreview() {
  ...
  // Subpage tags section
  csv += "SELECTED SUBPAGE TAGS\n";
  csv += "Name\n";
  if (appState.selectedSubpageTags.length > 0) {
    appState.selectedSubpageTags.forEach((tag) => {
      csv += `"${tag.name}"\n`;
    });
  } else {
    csv += "No subpage tags selected\n";
  }
  ...
  csv += `Total Subpage Tags Selected,${appState.selectedSubpageTags.length}\n`;
...
function updateAnalytics() {
  const total =
    ...
    appState.selectedSubpageTags.length;
  const maxTotal =
    appState.maxCuisines + appState.maxTags + appState.maxSubpageTags;
  ...
  document.getElementById("statSubpageTags").textContent =
    `${appState.selectedSubpageTags.length} / ${appState.maxSubpageTags}`;
...
function importData(e) {
  ...
  reader.onload = (event) => {
    try {
      ...
      appState.selectedSubpageTags = data.subpageTags || [];
      saveState();
      renderUI();
...
function loadStateFromStorage() {
  ...
  if (saved) {
    ...
    appState.selectedSubpageTags = (loaded.selectedSubpageTags || []).filter(
      (t) => cuisineTagData.subpageTags.find((x) => x.id === t.id),
    );
  }
